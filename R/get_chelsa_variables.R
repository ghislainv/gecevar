get_chelsa_variables <- function(extent, EPSG, destination, resolution = 1000, rm_download = FALSE){
  ...
  ###
  # file.remove(list.files(paste(destination, "data_raw", "chelsa_v2_1", "temp", sep = "/"), pattern = "_1km", full.names = TRUE))
  ###

  # Stack Tasmin, Tasmax, Tas, Pr, Tcc, Pet Penman & bio
  files.tif <- paste(destination, "data_raw", "chelsa_v2_1", paste(c("tasmin","tasmax","tas_","pr", "tcc", "pet_penman", "bio"),"_1km.tif", sep = "/"))
  ...
  for (i in 1:12)
  {
    ...
    system(glue('gdal_calc.py -A {pet_penman_file} --A_band={i} -B {pr_file} --B_band={i} --quiet --type=Int16 \\
               ...
               --outfile={paste(destination, "data_raw", "chelsa_v2_1", paste0("cwd", i, "_1km.tif"), sep = "/")} --overwrite'))
  ...
  system(glue('gdal_merge.py -o {paste(destination, "output", "current_chelsa.tif", sep = "/")} -of GTiff -ot Int16 -co "COMPRESS=LZW" \\
            -co "PREDICTOR=2" -separate -a_nodata {nodat} {paste(destination, "data_raw", "chelsa_v2_1", "clim_1km.tif", sep = "/")} \\
            {paste(destination, "data_raw", "chelsa_v2_1", "cwd_1km.tif", sep = "/")} {paste(destination, "data_raw", "chelsa_v2_1", "ndm_1km.tif", sep = "/")}'))
