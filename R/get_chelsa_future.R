get_chelsa_future <- function(extent, extent_latlon, EPSG, destination, resolution = 1000, phase = "2071-2100", ssp = 585){
  ...
  dir.create(paste(destination, "data_raw","future_chelsa", sep = "/"), showWarnings = FALSE) ## folder for climatic data
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "GFDL-ESM4", "ssp", ssp, sep = "_"), "temp", sep = "/"), showWarnings = FALSE, recursive = TRUE)
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "IPSL-CM6A-LR", "ssp", ssp, sep = "_"), "temp", sep = "/"), showWarnings = FALSE, recursive = TRUE)
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "MPI-ESM1-2-HR", "ssp", ssp, sep = "_"), "temp", sep = "/"), showWarnings = FALSE, recursive = TRUE)
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "MRI-ESM2-0", "ssp", ssp, sep = "_"), "temp", sep = "/"), showWarnings = FALSE, recursive = TRUE)
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "UKESM1-0-LL", "ssp", ssp, sep = "_"), "temp", sep = "/"), showWarnings = FALSE, recursive = TRUE)
  dir.create(paste(destination, "data_raw","future_chelsa", paste("climat", phase, "average", "ssp", ssp, sep = "_"),  sep = "/"), showWarnings = FALSE, recursive = TRUE)
  progress_bar <- 0
  ...
  for (model in c("GFDL-ESM4", "IPSL-CM6A-LR", "MPI-ESM1-2-HR", "MRI-ESM2-0", "UKESM1-0-LL")){
    for(m in str_pad(1:12, width = 2, pad = "0")){
      ...
      system(glue("gdal_translate -projwin {extent_latlon[1]} {extent_latlon[4]} {extent_latlon[3]} {extent_latlon[2]} -projwin_srs EPSG:4326 \\
                ...
                {paste(destination, 'data_raw', 'future_chelsa', paste('climat', phase, model, 'ssp', ssp, sep = '_'), 'temp', paste0('tasmin', m, '.tif'), sep = '/')}"), ignore.stdout = TRUE, ignore.stderr = TRUE)
      ...
      system(glue("gdal_translate -projwin {extent_latlon[1]} {extent_latlon[4]} {extent_latlon[3]} {extent_latlon[2]} -projwin_srs EPSG:4326 \\
                ...
                {paste(destination, 'data_raw', 'future_chelsa', paste('climat', phase, model, 'ssp', ssp, sep = '_'), 'temp', paste0('tasmax', m, '.tif'), sep = '/')}"), ignore.stdout = TRUE, ignore.stderr = TRUE)
      progress_bar <- progress_bar + 1
      setTxtProgressBar(pb, progress_bar)
      ...
      system(glue("gdal_translate -projwin {extent_latlon[1]} {extent_latlon[4]} {extent_latlon[3]} {extent_latlon[2]} -projwin_srs EPSG:4326 \\
                ...
                {paste(destination, 'data_raw', 'future_chelsa', paste('climat', phase, model, 'ssp', ssp, sep = '_'), 'temp', paste0('tas', m, '.tif'), sep = '/')}"), ignore.stdout = TRUE, ignore.stderr = TRUE)
      progress_bar <- progress_bar + 1
      setTxtProgressBar(pb, progress_bar)
      ...
      system(glue("gdal_translate -projwin {extent_latlon[1]} {extent_latlon[4]} {extent_latlon[3]} {extent_latlon[2]} -projwin_srs EPSG:4326 \\
                ...
                {paste(destination, 'data_raw', 'future_chelsa', paste('climat', phase, model, 'ssp', ssp, sep = '_'), 'temp', paste0('pr', m, '.tif'), sep = '/')}"), ignore.stdout = TRUE, ignore.stderr = TRUE)
      progress_bar <- progress_bar + 1
      setTxtProgressBar(pb, progress_bar)
    }
    ...
    for(i in 1:19){
      ...
      system(glue("gdal_translate -projwin {extent_latlon[1]} {extent_latlon[4]} {extent_latlon[3]} {extent_latlon[2]} -projwin_srs EPSG:4326 \\
                ...
                {paste(destination, 'data_raw', 'future_chelsa', paste('climat', phase, model, 'ssp', ssp, sep = '_'), 'temp', paste0('bio', i, '.tif'), sep = '/')}"), ignore.stdout = TRUE, ignore.stderr = TRUE)
      progress_bar <- progress_bar + 1
      setTxtProgressBar(pb, progress_bar)
    }
    close(pb)
  ...
  for (model in c("GFDL-ESM4", "IPSL-CM6A-LR", "MPI-ESM1-2-HR", "MRI-ESM2-0", "UKESM1-0-LL")){
    ...
    {
      files.tif <- list.files(paste(destination, "data_raw", "future_chelsa", paste('climat', phase, model, 'ssp', ssp, sep = '_'), "temp", sep = "/"), pattern = paste0(var, "[0-9]"), full.names = TRUE)
      ...
      write_stars(obj = r, options = c("COMPRESS=LZW","PREDICTOR=2"),
                  type = "Int16", dsn = paste(destination, "data_raw","future_chelsa", paste('climat', phase, model, 'ssp', ssp, sep = '_'), paste0(var,"_res.tif"), sep = "/"))
    }

    # Stack Tasmin, Tasmax, Tas, Pr & bio
    files.tif <- paste(destination, "data_raw", "future_chelsa", paste('climat', phase, model, 'ssp', ssp, sep = '_'), paste0(c("tasmin","tasmax","tas","pr", "bio"), "_res.tif"), sep = "/")
    ...
    write_stars(obj = r, options = c("COMPRESS=LZW","PREDICTOR=2"), NA_value = nodat,
                type = "Int16", dsn = paste(destination, "data_raw","future_chelsa", paste('climat', phase, model, 'ssp', ssp, sep = '_'), "clim_res.tif", sep = "/"))
    rm(r)
    ...
    pr <- read_stars(paste(destination, "data_raw", "future_chelsa", paste('climat', phase, model, 'ssp', ssp, sep = '_'), "pr_res.tif", sep = "/"))
