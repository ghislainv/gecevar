...
This chunk code take around 3min to run. 

## Download climatic variables in one file

Climatic variables are extract from [chelsa-climat.org](https://chelsa-climate.org/). 

We get with `get_chelsa_current` function many variables. You can find the chelsa's documentation [here](https://chelsa-climate.org/wp-admin/download-page/CHELSA_tech_specification_V2.pdf) about all variables downloaded.
Variables give the average value on the data recovered between 1981 and 2010.
Some variables are available for each month like average temperature, min temperature, max temperature, precipitation, potential evapotranspiration (PET) with Penman's formula and Thornthwaite's formula, and cloud area fraction (clt).
...
This function takes around 10min to run.

```{r env_clim, eval = FALSE}
climat_path <- get_chelsa_current(extent = extent,
                                      EPSG = epsg,
                                      destination = "/home/pierre/Bureau/NC",
                                      resolution = 1000,
                                      rm_download = FALSE)
...
In our case, gecevar.tif is a 35Mb weight file.
Output is a absolute path to gecevar.tif file whom just been created. 

## Download futur climat 

Extracted from chelsa the climate data from the five available models, namely, GFDL-ESM4, IPSL-CM6A-LR, MPI-ESM1-2-HR, MRI-ESM2-0 and UKESM1-0-LL.
Compute the average of these models for a SSP choosen. 
This function take around 40min to run.
